...
async def generate(ctx, *, args):
    ...
    aspect_ratio = parts[1].strip() if len(parts) > 1 else "16:9"

    # Translation to English
    response = translate_model.generate_content(prompt)

    try:
        prompt = response.candidates[0].text
    except KeyError as ke:
        await ctx.send(f"Failed to translate prompt: {str(ke)}")
    except Exception as e:
        await ctx.send(f"An error occurred during translation: {str(e)}")
        # print(f"Failed to translate prompt: {str(e)}")


    await ctx.send(f"Generating image for prompt (English): {prompt}")
